...
//		An OSErr code.
//
- (OSErr) sendWithPriority:(UInt32)__unused priority andTimeout:(UInt32)timeout
{
	// send the apple event without waiting for a reply
	return (OSErr)AESendMessage([self aeDesc], NULL, kAENoReply, timeout);
}
...
//		nil if error occurs.
//
- (NSAppleEventDescriptor*) sendExpectingReplyWithPriority:(UInt32)__unused priority
																andTimeout:(UInt32)timeout
{
	...
	OSStatus err = AESendMessage([self aeDesc], &resultDesc, kAEWaitReply, timeout);
