/**
    ...
    HISTORY :
        ...
        v.1.3 : september 2022, support for arm, http://vboehm.net
    ...
    we use a custom modified version of the objective max framework.
...
#import <ApplicationServices/ApplicationServices.h>     // vb
...
#define S2MWACOM_VERSION "1.3 -- vb"
...
void ext_main(void *r)
{
    ...
    post ("[s2m.wacom] version %s", S2MWACOM_VERSION); //  - %s , __DATE__
...
- (id) initWithObject:(t_object *)x name:(t_symbol *)s numArgs:(long)argc andValues:(t_atom *)argv;
{
    ...
    observedTypes =    NSEventMaskTabletPoint
 ...
 */
    DBG (@"s2m.wacom initialized");
        
    eventMonLocal = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskTabletProximity
                                          handler:^(NSEvent* theEvent)
                                          {
                                              [self manageProximityEvent:theEvent];
                                              return theEvent;
                                          }];
    eventMonGlobal = [NSEvent addGlobalMonitorForEventsMatchingMask:NSEventMaskTabletProximity
                                      handler:^(NSEvent* theEvent)
                                      {
                                          [self manageProximityEvent:theEvent];
                                      }];
        ...
        if (selectedMenuIndex > 0) // if not "all tablets and tools"
        {
            ...
            for (Tablet *tab in tablets)
            {
                if (selectedMenuIndex <= ([tab menuIndex] + [[tab transducers] count]) )
                {
                    ...
                    DBG (@"-selectedMenuIndex = %li-", (long)selectedMenuIndex);
                    DBG (@"-selectedTabletIndex = %i-", i);
                    DBG (@"-selectedToolIndex = %li-", (long)selectedToolIndex);
                    DBG (@"-[tab menuIndex] = %lu-", (unsigned long)[tab menuIndex]);
            ...
            DBG (@"-wantedTabletSystemID = %li-", (long)wantedTabletSystemID);
            DBG (@"-wantedToolUniqueID = %llu-", wantedToolUniqueID);
...
- (void) movesCursor:(BOOL) moveEnable
{
    ...
    if (selectedMenuIndex == 0 ) // "all tablets" selected
    {
        for (Tablet *tab in tablets)
        {
            ...
            BOOL success = [WacomTabletDriver setBytes:&newMovesCursor
...
//        [[outputAreaDesc data] getBytes:&outputArea];         // vb, deprecated and unsafe
        [[outputAreaDesc data] getBytes:&outputArea length:100];        // TODO: vb, ok???
...
- (void) outputValuesFromEvent:(NSEvent *)theEvent
{
    ...
    if ( eventType == NSEventTypeTabletPoint //NSTabletPoint
        ||    ( isMouseEvent == YES
             &&    [theEvent subtype] == NSEventSubtypeTabletPoint //NSTabletPointEventSubtype
            )
       )
    {
        ...
        if ( outputThatEvent )
        {
            ...
            if (nofilterAttribute)
            {
                ...
                {
                    atom_setfloat (at, absoluteX / (double) tabletXdimension);
                    atom_setfloat (at+1, absoluteY / (double) tabletYdimension);
                }
                else if (xyrangeAttribute == 2)
                {
                    atom_setfloat (at, absoluteX / (double) tabletXdimension);
                    atom_setfloat (at+1, absoluteY / (double) tabletXdimension);
                }
                else if (xyrangeAttribute == 3)
                {
                    atom_setfloat (at, absoluteX  * tabletXdimensionMm / (double) tabletXdimension);
                    atom_setfloat (at+1, absoluteY * tabletYdimensionMm / (double) tabletYdimension);
                }
    ...
    else if (mouseAttribute && (isMouseEvent || (eventType == NSEventTypeScrollWheel ) ) )
    {
    ...
    if (eventType == NSEventTypeTabletProximity) //NSTabletProximity)
        [self manageProximityEvent:theEvent];
